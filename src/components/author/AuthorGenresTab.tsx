...
export const AuthorGenresTab: React.FC<AuthorGenresTabProps> = ({ onFeedback }) => {
  ...
  const handleReset = async () => {
    if (window.confirm('Bạn có chắc chắn muốn khôi phục danh sách thể loại về mặc định ban đầu không?')) {
  ...
  return (
    <div id="author-genres-tab" className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-emerald-50 via-teal-50/80 to-pink-50/80 dark:from-stone-800 dark:via-stone-850 dark:to-stone-800 border border-emerald-200/90 dark:border-stone-700 shadow-2xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={handleReset}
            className="self-start sm:self-center px-3 py-1.5 rounded-xl border border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-800 hover:bg-stone-50 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-200 text-xs font-medium flex items-center gap-1.5 transition-colors shadow-2xs cursor-pointer shrink-0"
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-stone-800/95 border border-stone-200 dark:border-stone-700 shadow-2xs">
      ...
      {/* List of Genres */}
      <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-stone-800/95 border border-stone-200 dark:border-stone-700 shadow-2xs space-y-4">
      ...
      {genreToDelete && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-2xs flex items-center justify-center p-4 animate-in fade-in">
          <div className="w-full max-w-md p-6 rounded-2xl bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 shadow-xl space-y-4">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl border border-stone-300 dark:border-stone-600 text-stone-700 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-700 text-xs font-semibold cursor-pointer"
              >
                Hủy bỏ
              </button>
              <button
